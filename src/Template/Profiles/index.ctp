              ...
              <tbody>
                <?php foreach ($profiles as $profile): ?>
                      <tr role="row">
                        ...
                        <td class="text-right">
                          <a onclick="system.showSwal('Eliminar','¿Eliminar el perfil '+'<?= $profile->name ?>'+' ?','delete', '<?= $this->Url->build('/', true) ?><?= strtolower($this->request->getParam('controller')) ?>/delete/<?= $profile->id; ?>')" class="btn btn-round btn-danger btn-icon btn-sm text-white" rel="tooltip" title="Eliminar" data-placement="left"><i class="fas fa-trash"></i></a>
